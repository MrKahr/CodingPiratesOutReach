...
\frametitle{Hvad laver vi? (i GameDev)}
    \begin{center}
    \begin{tikzpicture}
        % procedure/command not placed in preamble because it is only used here!
        \newcommand{\nodenames}{
            {Engine, Logik, Assets, Design, Historiefortælling}
        }
        % I gave up on writing imperative TeX, screw it - we're going declarative babeee! 
        \graph[
            circular placement, 
            radius=6em,
            edges={draw, <->, shorten >=1pt}, 
            nodes={draw, align=center, rounded corners, minimum width=4em}
            ]
            {Engine, Logik, Assets, Design, Historiefortælling -> {Engine, Logik, Assets, Design, Historiefortælling}};    
    \end{tikzpicture}
    \end{center}
        % \begin{itemize}
